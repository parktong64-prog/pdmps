...
export default function SimulationClient() {
  ...
  const [afterPhotos, setAfterPhotos] = useState<Record<Angle, string | null>>({
    left: null,
    front: null,
    right: null,
  });
  const [afterErrors, setAfterErrors] = useState<Record<Angle, string | null>>({
    left: null,
    front: null,
    right: null,
  });
  ...
  const anyFailed = ANGLES.some((a) => afterErrors[a.key]);
  ...
  async function handleMake() {
    ...
    const nextAfter: Record<Angle, string | null> = { left: null, front: null, right: null };
    const nextErrors: Record<Angle, string | null> = { left: null, front: null, right: null };
    for (const r of results) {
      nextAfter[r.key] = r.image;
      nextErrors[r.key] = r.error;
    }
    setAfterPhotos(nextAfter);
    setAfterErrors(nextErrors);
    setView("result");
  }
        ...
        {view === "processing" && (
          <div className="rounded-2xl border border-[var(--line)] bg-[var(--card-bg)] px-5 py-11 text-center">
            ...
            <p className="text-[0.84rem] text-[var(--ink-soft)]">
              AI가 세 장의 사진을 분석해 시뮬레이션을 만들고 있어요… (최대 1분 정도 걸릴 수 있어요)
            </p>
          </div>
        )}
          ...
          <div>
            <div className="rounded-2xl border border-[var(--line)] bg-[var(--card-bg)] p-[18px]">
              ...
              <div className="mt-5">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {ANGLES.map((a) => {
                    const generated = afterPhotos[a.key];
                    const error = afterErrors[a.key];
                    return (
                      <div key={a.key} className="relative aspect-[3/4] overflow-hidden rounded-lg bg-[var(--line)]">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img
                          src={generated ?? photos[a.key] ?? undefined}
                          alt={`AI 예상 ${a.label}`}
                          className={`absolute inset-0 h-full w-full object-cover ${
                            generated ? "" : "brightness-[1.05] contrast-[1.05] saturate-[1.08]"
                          }`}
                        />
                        <span className="absolute bottom-1.5 left-1.5 rounded-full bg-black/55 px-1.5 py-0.5 font-[family-name:var(--font-mono-kr)] text-[0.58rem] font-semibold text-white">
                          {a.label}
                        ...
                        {error && (
                          <span className="absolute top-1.5 right-1.5 flex h-[18px] w-[18px] items-center justify-center rounded-full bg-[var(--warn-ink)] text-[0.62rem] text-white" title={error}>
                            !
                          </span>
                        )}
                      </div>
              ...
              {anyFailed && (
                <p className="mt-3.5 text-[0.7rem] leading-[1.55] text-[var(--warn-ink)]">
                  일부 사진은 AI 생성에 실패해 원본에 색 보정만 적용해 보여드렸어요 (사진 각도를 다시 확인하고
                  재시도해보세요).
                </p>
              )}
            </div>
