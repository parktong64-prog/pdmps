...
const PROMPTS: Record<SimulationAngle, string> = {
  front:
    "This is a medical face-lift consultation simulation photo. Edit this portrait to show the subtle, realistic result of a surgical face lift where the mid-face and cheek tissue has been repositioned along an upward-oblique vector (about 45 degrees, from the nasolabial fold area toward the temple). Reduce the depth of the nasolabial folds (smile lines) and reduce cheek sagging so the cheeks look naturally lifted and firmer. Keep the person's identity, facial proportions, skin tone, hairstyle, expression, background, lighting, and camera angle exactly the same. Do not add makeup, do not change apparent age drastically, do not alter eyes/nose/mouth shape. Photorealistic, subtle, natural-looking result — not exaggerated or cartoonish.",
  left:
